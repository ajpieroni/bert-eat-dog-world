...
def dev_test_train_split(filename):
    ...
    for current_set in ['dev', 'test']:
        ...
        df_subset = pd.DataFrame(
                {'id': int(), 'text': str(), 'label': int()}, index=[])
        for i in df.word.unique():
            ...
            dg_idx = np.random.choice(
                    dg.index, size=dogwhistle_rel, replace=False)
            c_idx = np.random.choice(
                    c.index, size=conventional_rel, replace=False)
...
if __name__ == '__main__':
    ...
    X_dev, y_dev, X_test, y_test, X_train, y_train = \
        dev_test_train_split(sys.argv[1])
    for i in ['train', 'test', 'dev']:
        df = pd.DataFrame({'text': eval(f'X_{i}'), 'label': eval(f'y_{i}')},
                          columns=['text', 'label'])
